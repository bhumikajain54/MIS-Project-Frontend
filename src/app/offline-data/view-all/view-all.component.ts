...
// viewPolicyDetails(_t144: any) {
//   console.log('Viewing policy:', this.policies);
// }
  title = 'Offline Policy Management';
  keyword: string = '';
  productType: string = '';
  region: string = '';
  posp: string = '';
...
RM: any;
      ...
      this.apiService.getPolicies().subscribe(
        (data) => {
          if (data && Array.isArray(data)) {
            ...
            this.posts = []; // Fallback to empty array if data is not valid
          }
        },
        (error) => {
  ...
  //     (response) => {
  //       console.log('Import Policies Response:', response);
  //     },
  //     (error) => {
